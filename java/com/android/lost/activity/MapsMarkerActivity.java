...
public class MapsMarkerActivity extends AppCompatActivity
    ...
    public void onMapReady(GoogleMap googleMap) {
        ...
        // [END_EXCLUDE]
        // 获取所有的数据，并进行标记，但是把屏幕的镜头只移动到第一个记录上
        List<LostAndFound> list = DAOService.getInstance().searchHealthInfoByStartAndEnd();
        for(LostAndFound lostAndFound : list) {
            googleMap.addMarker(new MarkerOptions()
                    .position(new LatLng(lostAndFound.getLatitude(), lostAndFound.getLongitude()))
                    .title(lostAndFound.getLocation()));
        }
        // [START_EXCLUDE silent]
        if(Utils.isNotEmpty(list)) {
            googleMap.moveCamera(CameraUpdateFactory.newLatLngZoom(new LatLng(list.get(0).getLatitude(), list.get(0).getLongitude()),15));
        }
        // [END_EXCLUDE]
    }
